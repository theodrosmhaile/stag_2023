...
mean.acc.3 %<>% t()
mean.acc.6 %<>% t()
colnames(mean.acc.3) = paste0('ns3.',1:12)
colnames(mean.acc.6) = paste0('ns6.',1:12)
rlwm_data <-  cbind(mean.acc.3, mean.acc.6) %>% 
  as_data_frame()
 
  
ns3_test <- rep(mean.test.3,12) %>% 
    as.matrix() %>% 
    reshape(., 121,12) %>%
    as_data_frame()
  
colnames(ns3_test) <- paste0('nt3.', 1:12)

  ns6_test <-  rep(mean.test.6,12) %>% 
    as.matrix() %>% 
    reshape(., 121,12) %>%
    as_data_frame()
colnames(ns6_test) <- paste0('nt6.', 1:12)
