...
LTM.sim <- fromJSON('./simulated_data/LTM_model/LTM_sim_data_BLL_short_break_12_trace.JSON')$data %>% #LTM_sim_data_02202021.JSON 
  dplyr::mutate(bias = 0 )
...
unite(col='cond.model', c( 'condition','type'),remove = F) %>% #'condition'
 filter(model=='LTM') %>% 
   ggplot(aes(as.numeric(iteration),accuracy, group=cond.model,color=cond.model)) +
  geom_point(size=1.5) +
  geom_line(size=1) +
  scale_color_brewer(palette = "Paired") +
...
```{r learn to test change}
 p.model.behav.dat %>% 
  filter(iteration=='12') %>%
unite(col='cond.model', c( 'condition','type'),remove = F) %>% #'condition'
 filter(model=='LTM') %>% 
   ggplot(aes(phase,accuracy, group=cond.model,color=cond.model)) +
  geom_point(size=1.5) +
  geom_line(size=1) +
  scale_color_brewer(palette = "Paired") +
  facet_wrap(vars(subjects))
...
```{r}
LL.fits = read_csv('~/Collins/LL_model1.csv')

alpha_cor = LL.fits %>%
  select(subject, alpha) %>%
  rename(subjects = subject) %>% 
  inner_join(param_idx %>% filter(parameter=='alpha') %>% drop_na(), by='subjects')
  
 alpha_cor %>% 
   ggplot(aes(x=alpha,y=values)) +
   geom_point()+
  geom_smooth(method = 'lm')
  
 
decay_corr = LL.fits %>%
  select(subject, decay) %>%
  rename(subjects = subject) %>% 
  inner_join(param_idx %>% filter(parameter=='bll') %>% drop_na(), by='subjects')
 
  decay_corr %>% 
   ggplot(aes(x=decay,y=values)) +
   geom_point()+
  geom_smooth(method = 'lm')
```
...
```{r slimstampen}
 candy_dat <- read_delim('./behavioral_data_raw/slim/9856_Candy_responses.csv', delim = ';') %>% 
 rename("userId" = "user_id", "lessonId" = "lesson_id", 
        "factId" = "fact_id", "presentationStartTime" = "presentation_start_time", 
  ...
  mutate(factText = as.character(factId))
      
    
 maps_dat <- read_delim('./behavioral_data_raw/slim/9857_Maps_responses.csv', delim = ';') %>% 
   rename("userId" = "user_id", "lessonId" = "lesson_id", 
        "factId" = "fact_id", "presentationStartTime" = "presentation_start_time", 
        "presentationDuration" = "presentation_duration", 
        "sessionTime" = "session_time", 
  ...
  calculate_alpha_and_activation(candy_dat)
load(clean)



load('~/Blaarkop/cleandata_avg.RData')
